	...
	<div class="card card-outline card-success">
		<div class="card-header">
			<div class="card-tools">
				<a class="btn btn-block btn-sm btn-default btn-flat border-primary" href="./index.php?page=new_employee"><i class="fa fa-plus"></i> Agregar Nuevo Abogado</a>
			</div>
		</div>
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					...
					$designations = $conn->query("SELECT * FROM designation_list WHERE empresa='$empresa'");
					$design_arr[0]= "Unset";
					while($row=$designations->fetch_assoc()){
						$design_arr[$row['id']] =$row['designation'];
					}
					$departments = $conn->query("SELECT * FROM department_list where empresa='$empresa'");
					$dept_arr[0]= "Unset";
					while($row=$departments->fetch_assoc()){
						$dept_arr[$row['id']] =$row['department'];
					}
					$qry = $conn->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name FROM employee_list WHERE empresa='$empresa' order by concat(lastname,', ',firstname,' ',middlename) asc");
...
<script>
	...
	$('.view_employee').click(function(){
		uni_modal("<i class='fa fa-id-card'></i> Detalles del Abogado","view_employee.php?id="+$(this).attr('data-id'))
	})
	$('.delete_employee').click(function(){
	_conf("¿Estás seguro de eliminar a este abogado?","delete_employee",[$(this).attr('data-id')])
	})
	})
